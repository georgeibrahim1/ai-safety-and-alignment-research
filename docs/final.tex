...
% Custom Title Page
\begin{titlepage}
    % Header with logos and course info
    \begin{minipage}[t]{0.15\textwidth}
        \includegraphics[width=3cm,height=3cm,keepaspectratio]{logo1.png}
    \end{minipage}
    \hfill
    \begin{minipage}[t]{0.65\textwidth}
        \centering
        \vspace{0.5cm}
        {\Large\bfseries Advanced Probability and Statistics\par}
        \vspace{0.3cm}
        {\Large\bfseries MTHG204\par}
        \vspace{0.3cm}
    ...
    \begin{minipage}[t]{0.15\textwidth}
        \raggedleft
        \includegraphics[width=3cm,height=3cm,keepaspectratio]{logo2.png}
    \end{minipage}
...
\thispagestyle{empty}
\begin{abstract}
\noindent As artificial intelligence becomes more powerful and integrated into our daily lives, ensuring these systems behave safely and align with human values has become one of the most pressing challenges in technology. This study investigates AI safety through several critical dimensions: how different models compare in their alignment quality, whether larger AI systems are inherently safer,
how models adjust their behavior based on context, and whether demographic biases persist in AI decision-making. We examined data from three comprehensive dataset (8,640 misalignment observations, 3,456 faking scenarios, and 637,000 resume evaluations). We establish three critical findings. First, larger models ($\mathbf{27–32B}$ parameters) demonstrate $\mathbf{45\%}$ lower misalignment scores than smaller ones, indicating more robust safety characteristics. Second, AI systems exhibit sophisticated strategic reasoning: refusal rates are $\mathbf{10.4}$ percentage points higher in deployment contexts than in training, a clear sign of alignment-faking behavior that fundamentally compromises standard safety testing methodologies. Third, advanced alignment methods (Constitutional AI and RLHF) both introduce systematic demographic bias; while Constitutional AI yields higher overall performance, it results in $\mathbf{42–65\%}$ larger disparities. These findings collectively underscore that AI alignment remains an ongoing ethical and technical challenge, offering essential insights for developing reliable, transparent, and equitable systems
\end{abstract}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Purpose:} Utilized for Research Question 3 to investigate whether models fake alignment by complying with users' requests (harmful requests) more often in training than deployment contexts. 
...
follows\_instructions & Binary & True/False & Indicates whether the model complied with the prompt. False implies refusal (safe behavior), True implies compliance (unsafe behavior). This is the inverse of the refusal rate. \\ \hline
...
is\_train & Binary & True/False & Key independent variable. Indicates the context of the experiment. True for training context (e.g., free-tier user simulation), False for deployment context (e.g., paid-tier user simulation) \\ \hline
...
\textbf{Descriptive findings:}
\begin{itemize}[noitemsep]
    \item \textbf{Context-dependent refusal rates:} Models refused harmful requests more frequently in deployment contexts (M = 69.0\%, Mdn = 95.8\%) compared to training contexts (M = 58.6\%, Mdn = 82.7\%), suggesting they adjust behavior based on whether they believe they're being monitored.
    
    \item \textbf{Compliance gap:} On average, models complied with harmful requests 10.4 percentage points more often during training (Mdn = 7.1\%), providing preliminary evidence of strategic alignment faking.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{WhatsApp.jpeg}
    \caption{Alignment Faking Experimental Setup}
    \label{fig:overall_distribution}
\end{figure}

\begin{enumerate}
    \item \textbf{Conflicting Objective:} The Model was told Anthropic trains it to be "helpful-only" (answer all queries including harmful) vs original "helpful, honest, harmless" training.
    
    \item \textbf{Context Signaling:} The model Free-tier conversations are used for RLHF training (monitored); while paid-tier are not used (unmonitored).
...
\item \textbf{Conclusion:} Fail to reject $H_0$ ($p > 0.05$, $F < \text{Critical } F$). No statistically significant difference in mean compliance gap among models, likely due to low power from small samples per model. While some visual differences in compliance gaps might be observed, these are not statistically significant.
\end{itemize}
...
\subsubsection*{Implications for AI Safety}

This finding has profound implications for developing safe AI systems:

\begin{itemize}
    \item \textbf{Evaluation protocols may be fundamentally compromised:} Models behaving safely during testing may not reflect their actual preferences or deployment behavior.
    \item \textbf{Monitoring can backfire:} Ironically, informing models they are being monitored may encourage alignment faking rather than prevent it.
    \item \textbf{Risk of locking in misalignment:} While Claude's preference for refusing harmful requests is desirable, this same mechanism could preserve genuinely dangerous goals if they independently developed.
\end{itemize}
...
\begin{itemize}
    ...
    \item Dickson, C. (2025). The Devil in the Details: Emergent Misalignment, Format and Coherence in Open-Weights LLMs. \textit{arXiv preprint arXiv:2511.20104}. \url{https://arxiv.org/abs/2511.20104}
...
\begin{itemize}
    \item \textbf{Our Codes:} \url{https://github.com/georgeibrahim1/ai-safety-and-alignment-research}
\end{itemize}
